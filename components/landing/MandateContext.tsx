...
export function MandateContext() {
  return (
    <section className="border-b border-[#E5E7EB] bg-[#F9FAFB] px-6 py-[72px] dark:border-white/8 dark:bg-[#071A0E]">
      <div className="mx-auto max-w-[1100px]">
        <div className="grid items-center gap-16 md:grid-cols-2">
          <div>
            <p className="mb-3 text-xs font-semibold uppercase tracking-[1.2px] text-[#10B981]">Why now</p>
            <h2 className="font-[family-name:var(--font-plus-jakarta)] mb-4 text-[clamp(26px,3.5vw,36px)] font-bold tracking-[-0.6px] text-[#052E16] dark:text-white">
          ...
          <div className="flex flex-col gap-3.5">
            {STAGES.map((stage) => (
              <div
                key={stage.title}
                className="flex gap-3.5 rounded-xl border border-[#E5E7EB] bg-white px-5 py-[18px] dark:border-white/8 dark:bg-[#0F2318]"
              >
                <div
                  className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] ${stage.iconBgClass} dark:bg-white/8`}
                >
                ...
                <div>
                  <h4 className="mb-1 text-sm font-semibold text-[#052E16] dark:text-white">{stage.title}</h4>
